...
# Função para gerar dados aleatórios das bandas
def gerar_dados_bandas():
    dados_bandas = {}
    for banda in bandas:
        # Gerar nota aleatória (0 a 10)
        nota = random.randint(0, 10)
        
        # Gerar disponibilidade aleatória para cada dia da semana
        disponibilidade = {dia: random.choice([True, False]) for dia in dias_semana}
...
# Gerar dados aleatórios das bandas
print("="*40)
print(" GERANDO DADOS ALEATÓRIOS DAS BANDAS ")
print("="*40)
dados_bandas = gerar_dados_bandas()
...
bandas_casas_usadas = {banda: [] for banda in bandas}  # Casas em que cada banda já foi alocada
...
for casa, dia, dia_semana, horario in slots_disponiveis:
    ...
    bandas_disponiveis = [
        ...
        and casa not in bandas_casas_usadas[b]  # Não tocar na mesma casa consecutivamente
        and dados_bandas[b]["disponibilidade"][dia_semana]  # Banda está disponível no dia
    ]

    if not bandas_disponiveis:
        # Se não houver bandas disponíveis, relaxar a restrição de dias consecutivos
        bandas_disponiveis = [
            b for b in bandas
    ...
    # Escolher a banda com a maior pontuação e menos utilizada
    for banda in bandas_disponiveis:
        score = calcular_pontuacao(banda, dia_semana)
        # Priorizar bandas menos utilizadas
        if bandas_usadas[banda] < bandas_usadas.get(melhor_banda, float('inf')):
    ...
    if melhor_banda:
        ...
        bandas_casas_usadas[melhor_banda].append(casa)  # Registrar a casa em que a banda tocou
